...
class TestSystemdUnits(unittest.TestCase):
    def test_full_router_unit_uses_main_app_and_port_9091(self):
        unit = render_unit(
            name="Coding Agent Router",
            module="app.main",
            port=9091,
            state_subdir="app_server_full",
        )
        self.assertIn("Description=Coding Agent Router", unit)
        self.assertIn("ExecStart=/home/jesse/src/coding-agent-router/.venv/bin/uvicorn app.main:app --host 127.0.0.1 --port 9091", unit)
        self.assertIn("Environment=CODEX_CMD=/home/jesse/.nvm/versions/node/v22.13.1/bin/codex", unit)
        self.assertIn("Environment=APP_SERVER_STATE_DIR=/home/jesse/src/coding-agent-router/state/app_server_full", unit)
    ...
    def test_compaction_unit_uses_compaction_app_and_port_8081(self):
        unit = render_unit(
            ...
            port=8081,
            state_subdir="app_server_compaction",
        )
        ...
        self.assertIn("Environment=APP_SERVER_STATE_DIR=/home/jesse/src/coding-agent-router/state/app_server_compaction", unit)
